import {arrayUnion, runTransaction, setDoc} from 'firebase/firestore';
import {serverTimestamp, getFirebaseDb} from '../firebase/config';
import {posizioneLiveRef, servizioRef} from '../firebase/collections';
...
export function isWebGeolocationAvailable(): boolean { return 'geolocation' in navigator; }

export async function startSnowService(context: ActiveServiceContext) {
  if (!isWebGeolocationAvailable()) throw new Error('Geolocalizzazione non supportata dal browser');
  currentServiceStartedAt = new Date();
  currentServicePoints = [];
  await setDoc(servizioRef(context.aziendaId, context.servizioNeveId), {
    ...context,
...
async function savePosition(context: ActiveServiceContext, position: GeolocationPosition) {
  ...
  await runTransaction(getFirebaseDb(), async transaction => {
    transaction.set(posizioneLiveRef(context.aziendaId, context.operatoreId), {...context, punto, updatedAt: serverTimestamp(), createdAt: serverTimestamp()}, {merge: true});
    transaction.set(servizioRef(context.aziendaId, context.servizioNeveId), {...context, puntiGps: arrayUnion(punto), updatedAt: serverTimestamp()}, {merge: true});
  });
}
